...
export default class KubeService {
    ...
    constructor() {
        ...
        try {
            ...
            logger.error('Error while loading the Kubernetes configuration:', error)
            throw new Error('Invalid Kubernetes configuration')
        }

        if (process.env.NODE_ENV === 'development' || process.env.KUBE_SKIP_TLS_VERIFY === 'true') {
    ...
    async checkConnection(): Promise<boolean> {
        try {
            ...
            logger.error('Error while checking the connection to the Kubernetes cluster:', error)
            return false
        }
    }
    ...
    async createServer(
        ...
        const serverProperties = server.serverProperties as Record<string, string>
        ...
        try {
            ...
            logger.info(`Service created for server ${server.serverId} with node port ${nodePort}`)
